...
RSpec.describe 'Roles', type: :request do
  ...
  describe 'DELETE /roles/:id' do
    ...
    it 'does not delete reserved role like admin' do
  ...
  describe 'PUT /roles/:id' do
    ...
    it 'Do not allow system reserved role' do
      put api_v1_role_path(Role.find(1)), params: data
      expect(response).to have_http_status(:unprocessable_entity)
      expect(
        json.dig(:errors, 0)
      ).to eq("Name #{I18n.t('activerecord.errors.models.role.name_is_reserved', name: Role.find(1).name)}")
    end
  end
end
